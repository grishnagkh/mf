 ...
 */
package mf.com.google.android.exoplayer.chunk;
...
import mf.com.google.android.exoplayer.FormatHolder;
...
import mf.com.google.android.exoplayer.TrackRenderer;
import mf.com.google.android.exoplayer.upstream.DataSpec;
import mf.com.google.android.exoplayer.upstream.Loader;
...
public class ChunkSampleSource implements SampleSource, Loader.Listener {
	...
	public interface EventListener {
		 ...
		 * @param totalBytes
		 *            The length of the data being loaded in bytes.
		 */
		void onLoadStarted(int sourceId, String formatId, int trigger,
				boolean isInitialization, int mediaStartTimeMs,
				int mediaEndTimeMs, long totalBytes);
		 ...
		 */
		void onUpstreamDiscarded(int sourceId, int mediaStartTimeMs,
				int mediaEndTimeMs, long totalBytes);
		 ...
		 *            The length of the data being discarded in bytes.
		 */
		void onDownstreamDiscarded(int sourceId, int mediaStartTimeMs,
				int mediaEndTimeMs, long totalBytes);
		 ...
		 */
		void onDownstreamFormatChanged(int sourceId, String formatId,
	...
	public ChunkSampleSource(ChunkSource chunkSource, LoadControl loadControl,
		...
		this(chunkSource, loadControl, bufferSizeContribution,
				frameAccurateSeeking, null, null, 0);
	}
	...
	@Override
	public int readData(int track, long playbackPositionUs,
			FormatHolder formatHolder, SampleHolder sampleHolder,
		...
		if (mediaChunk.isReadFinished()) {
			...
			if (mediaChunks.size() > 1) {
				...
				return readData(track, playbackPositionUs, formatHolder,
						sampleHolder, false);
		...
		if (downstreamFormat == null
				|| !downstreamFormat.equals(mediaChunk.format)) {
			notifyDownstreamFormatChanged(mediaChunk.format.id,
					mediaChunk.trigger, mediaChunk.startTimeUs);
		...
		if (mediaChunk.read(sampleHolder)) {
			sampleHolder.decodeOnly = frameAccurateSeeking
	...
	public void seekToUs(long timeUs) {
		...
		if (mediaChunk == null) {
			...
			pendingDiscontinuity |= mediaChunk.seekTo(timeUs,
					mediaChunk == mediaChunks.getFirst());
		...
		if (currentLoadable != null && mediaChunk == currentLoadable) {
			...
			if (chunkLength != DataSpec.LENGTH_UNBOUNDED) {
				return mediaChunk.startTimeUs
						+ ((mediaChunk.endTimeUs - mediaChunk.startTimeUs) * mediaChunk
	...
	private void updateLoadControl() {
		...
		if (isPendingReset()) {
			...
			loadPositionUs = lastMediaChunk.nextChunkIndex == -1 ? -1
					: lastMediaChunk.endTimeUs;
		}

		boolean isBackedOff = currentLoadableException != null
				&& !currentLoadableExceptionFatal;
		boolean nextLoader = loadControl.update(this, downstreamPositionUs,
				loadPositionUs, isBackedOff || loader.isLoading(),
				currentLoadableExceptionFatal);
		...
		if (!loader.isLoading()) {
			if (currentLoadableHolder.chunk == null
					|| now - lastPerformedBufferOperation > 1000) {
				...
				chunkSource.getChunkOperation(readOnlyMediaChunks,
						pendingResetTime, downstreamPositionUs,
	...
	private void resumeFromBackOff() {
		...
		currentLoadableHolder.queueSize = readOnlyMediaChunks.size();
		chunkSource.getChunkOperation(readOnlyMediaChunks, pendingResetTime,
	...
	private void maybeStartLoading() {
		...
		if (isMediaChunk(currentLoadable)) {
			...
			notifyLoadStarted(mediaChunk.format.id, mediaChunk.trigger, false,
					mediaChunk.startTimeUs, mediaChunk.endTimeUs,
					mediaChunk.getLength());
		} else {
			notifyLoadStarted(currentLoadable.format.id,
					currentLoadable.trigger, true, -1, -1,
	...
	private void discardDownstreamMediaChunk() {
		...
		removed.release();
		notifyDownstreamDiscarded(removed.startTimeUs, removed.endTimeUs,
				totalBytes);
	}
	...
	private void notifyLoadStarted(final String formatId, final int trigger,
			final boolean isInitialization, final long mediaStartTimeUs,
			final long mediaEndTimeUs, final long totalBytes) {
		if (eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public void run() {
					eventListener.onLoadStarted(eventSourceId, formatId,
							trigger, isInitialization,
	...
	private void notifyUpstreamDiscarded(final long mediaStartTimeUs,
			final long mediaEndTimeUs, final long totalBytes) {
		if (eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public void run() {
					eventListener.onUpstreamDiscarded(eventSourceId,
							usToMs(mediaStartTimeUs), usToMs(mediaEndTimeUs),
							totalBytes);
				}
			});
	...
	private void notifyDownstreamFormatChanged(final String formatId,
			final int trigger, final long mediaTimeUs) {
		if (eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public void run() {
					eventListener.onDownstreamFormatChanged(eventSourceId,
							formatId, trigger, usToMs(mediaTimeUs));
				}
			});
		}
	}

	private void notifyDownstreamDiscarded(final long mediaStartTimeUs,
			final long mediaEndTimeUs, final long totalBytes) {
		if (eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public void run() {
					eventListener.onDownstreamDiscarded(eventSourceId,
							usToMs(mediaStartTimeUs), usToMs(mediaEndTimeUs),
							totalBytes);
				}
			});
