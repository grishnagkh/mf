 ...
 */
package mf.com.google.android.exoplayer.text;
...
import mf.com.google.android.exoplayer.dash.mpd.AdaptationSet;
import mf.com.google.android.exoplayer.util.Assertions;
import mf.com.google.android.exoplayer.util.VerboseLogUtil;
...
public class TextTrackRenderer extends TrackRenderer implements Callback {
	 ...
	 */
	public TextTrackRenderer(SampleSource source,
			SubtitleParser subtitleParser, TextRenderer textRenderer,
	...
	private void seekToInternal(long timeUs) {
		...
		source.seekToUs(timeUs);
		if (subtitle != null
				&& (timeUs < subtitle.getStartTime() || subtitle
						.getLastEventTime() <= timeUs)) {
			subtitle = null;
		}
		resetSampleData();
		clearTextRenderer();
	...
	protected void doSomeWork(long timeUs) throws ExoPlaybackException {
		...
		if (subtitle == null) {
			...
			try {
				int result = source.readData(trackIndex, timeUs, formatHolder,
						sampleHolder, false);
				if (result == SampleSource.SAMPLE_READ) {
					resetSampleHolder = true;
					InputStream subtitleInputStream = new ByteArrayInputStream(
							sampleHolder.data.array(), 0, sampleHolder.size);
					subtitle = subtitleParser.parse(subtitleInputStream,
							"UTF-8", sampleHolder.timeUs);
					syncNextEventIndex(timeUs);
					textRendererNeedsUpdate = true;
	...
	private void syncNextEventIndex(long timeUs) {
		nextSubtitleEventIndex = subtitle == null ? -1 : subtitle
				.getNextEventTimeIndex(timeUs);
	}

	private long getNextEventTime() {
		return ((nextSubtitleEventIndex == -1) || (nextSubtitleEventIndex >= subtitle
				.getEventTimeCount())) ? Long.MAX_VALUE : (subtitle
				.getEventTime(nextSubtitleEventIndex));
	}
	...
	private void updateTextRenderer(long timeUs) {
		...
		if (textRendererHandler != null) {
			textRendererHandler.obtainMessage(MSG_UPDATE_OVERLAY, text)
					.sendToTarget();
		} else {
	...
	private void clearTextRenderer() {
		...
		if (textRendererHandler != null) {
			textRendererHandler.obtainMessage(MSG_UPDATE_OVERLAY, null)
					.sendToTarget();
		} else {
