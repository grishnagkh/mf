...
public class MediaCodecAudioTrackRenderer extends MediaCodecTrackRenderer {
	...
	public static class AudioTrackInitializationException extends Exception {
		...
		public AudioTrackInitializationException(int audioTrackState,
				int sampleRate, int channelConfig, int bufferSize) {
			super("AudioTrack init failed: " + audioTrackState + ", Config("
	...
	public MediaCodecAudioTrackRenderer(SampleSource source,
		...
		this(source, drmSessionManager, playClearSamplesWithoutKeys,
				DEFAULT_MIN_BUFFER_MULTIPLICATION_FACTOR, eventHandler,
				eventListener);
	}
	 ...
	 */
	public MediaCodecAudioTrackRenderer(SampleSource source,
			float minBufferMultiplicationFactor, Handler eventHandler,
			EventListener eventListener) {
		this(source, null, true, minBufferMultiplicationFactor, eventHandler,
				eventListener);
	}
	...
	public MediaCodecAudioTrackRenderer(SampleSource source,
		...
		if (Util.SDK_INT >= 18) {
			try {
				audioTrackGetLatencyMethod = AudioTrack.class.getMethod(
						"getLatency", (Class<?>[]) null);
	...
	protected void onOutputFormatChanged(MediaFormat format) {
		...
		this.minBufferSize = AudioTrack.getMinBufferSize(sampleRate,
	...
	protected boolean isEnded() {
		...
		return super.isEnded()
				&& (getPendingFrameCount() == 0 || submittedBytes < minBufferSize);
	}
	...
	protected long getCurrentPositionUs() {
		...
		if (audioTrack == null
			...
			// in the future)
			long presentationDiff = systemClockUs
					- (audioTimestampCompat.getNanoTime() / 1000);
			long framesDiff = durationUsToFrames(presentationDiff);
			// The position of the frame that's currently being presented.
			long currentFramePosition = audioTimestampCompat.getFramePosition()
					+ framesDiff;
			currentPositionUs = framesToDurationUs(currentFramePosition)
					+ audioTrackStartMediaTimeUs;
		} else {
			if (playheadOffsetCount == 0) {
				// The AudioTrack has started, but we don't have any samples to
				// compute a smoothed position.
				currentPositionUs = getPlayheadPositionUs()
						+ audioTrackStartMediaTimeUs;
			} else {
				// getPlayheadPositionUs() only has a granularity of ~20ms, so
				// we base the position off the
				// system clock (and a smoothed offset between it and the
				// playhead position) so as to
				// prevent jitter in the reported positions.
				currentPositionUs = systemClockUs + smoothedPlayheadOffsetUs
						+ audioTrackStartMediaTimeUs;
			}
			if (!isEnded()) {
		...
		currentPositionUs = Math.max(lastReportedCurrentPositionUs,
				currentPositionUs);
	...
	private void maybeSampleSyncParams() {
		...
		if (systemClockUs - lastPlayheadSampleTimeUs >= MIN_PLAYHEAD_OFFSET_SAMPLE_INTERVAL_US) {
			...
			playheadOffsets[nextPlayheadOffsetIndex] = playheadPositionUs
					- systemClockUs;
			nextPlayheadOffsetIndex = (nextPlayheadOffsetIndex + 1)
			...
			for (int i = 0; i < playheadOffsetCount; i++) {
				smoothedPlayheadOffsetUs += playheadOffsets[i]
						/ playheadOffsetCount;
			}
		}
		...
		if (systemClockUs - lastTimestampSampleTimeUs >= MIN_TIMESTAMP_SAMPLE_INTERVAL_US) {
			...
			if (audioTrackGetLatencyMethod != null) {
				try {
					...
					audioTrackLatencyUs = (Integer) audioTrackGetLatencyMethod
							.invoke(audioTrack, (Object[]) null)
							* 1000L
	...
	@Override
	protected boolean processOutputBuffer(long timeUs, MediaCodec codec,
			ByteBuffer buffer, MediaCodec.BufferInfo bufferInfo,
			int bufferIndex, boolean shouldSkip) throws ExoPlaybackException {
		if (shouldSkip) {
		...
		if (temporaryBufferSize == 0) {
			...
			if (audioTrackStartMediaTimeState == START_NOT_SET) {
				...
				if (audioTrackStartMediaTimeState == START_IN_SYNC
					...
					Log.e(TAG, "Discontinuity detected [expected "
							+ expectedBufferStartTime + ", got "
							+ bufferStartTime + "]");
					audioTrackStartMediaTimeState = START_NEED_SYNC;
				}
			...
			if (temporaryBuffer == null
					|| temporaryBuffer.length < bufferInfo.size) {
		...
		int bytesPending = (int) (submittedBytes - getPlaybackHeadPosition()
				* frameSize);
		int bytesToWrite = bufferSize - bytesPending;

		if (bytesToWrite > 0) {
			bytesToWrite = Math.min(temporaryBufferSize, bytesToWrite);
			audioTrack.write(temporaryBuffer, temporaryBufferOffset,
					bytesToWrite);
	...
	private long getPlaybackHeadPosition() {
		long rawPlaybackHeadPosition = 0xFFFFFFFFL & audioTrack
	...
	private int getPendingFrameCount() {
		return audioTrack == null ? 0
				: (int) (submittedBytes / frameSize - getPlaybackHeadPosition());
	}
	...
	private void notifyAudioTrackInitializationError(
			final AudioTrackInitializationException e) {
	 ...
	 * throws an exception.
	 */
	private static final class NoopAudioTimestampCompat implements
	 ...
	 */
	@TargetApi(19)
	private static final class AudioTimestampCompatV19 implements
			AudioTimestampCompat {
