...
public abstract class MediaCodecTrackRenderer extends TrackRenderer {
	...
	public static class DecoderInitializationException extends Exception {
		...
		public DecoderInitializationException(String decoderName,
				MediaFormat mediaFormat, Exception cause) {
			super("Decoder init failed: " + decoderName + ", " + mediaFormat,
					cause);
	 ...
	 *            required.
	 */
	public MediaCodecTrackRenderer(SampleSource source,
	...
	protected void configureCodec(MediaCodec codec,
			android.media.MediaFormat x, MediaCrypto crypto) {
	...
	protected final void maybeInitCodec() throws ExoPlaybackException {
		...
		if (drmInitData != null) {
			...
			if (drmSessionState == DrmSessionManager.STATE_ERROR) {
				...
				requiresSecureDecoder = drmSessionManager
						.requiresSecureDecoderComponent(mimeType);
			} else {
		...
		DecoderInfo selectedDecoderInfo = MediaCodecUtil
				.getDecoderInfo(mimeType);
		...
		codecHotswapTimeMs = getState() == TrackRenderer.STATE_STARTED ? SystemClock
				.elapsedRealtime() : -1;
		inputIndex = -1;
		outputIndex = -1;
	...
	protected long getBufferedPositionUs() {
		...
		return sourceBufferedPosition == UNKNOWN_TIME
				|| sourceBufferedPosition == END_OF_TRACK ? sourceBufferedPosition
	...
	protected void doSomeWork(long timeUs) throws ExoPlaybackException {
		try {
			...
			if (format == null) {
				readFormat();
			} else if (codec == null && !shouldInitCodec()
					&& getState() == TrackRenderer.STATE_STARTED) {
	...
	private void readFormat() throws IOException, ExoPlaybackException {
		int result = source.readData(trackIndex, currentPositionUs,
				formatHolder, sampleHolder, false);
		if (result == SampleSource.FORMAT_READ) {
			onInputFormatChanged(formatHolder);
		}
	}

	private void discardSamples(long timeUs) throws IOException,
			ExoPlaybackException {
		sampleHolder.data = null;
		int result = SampleSource.SAMPLE_READ;
		while (result == SampleSource.SAMPLE_READ
				&& currentPositionUs <= timeUs) {
			result = source.readData(trackIndex, currentPositionUs,
					formatHolder, sampleHolder, false);
	...
	private void checkForDiscontinuity() throws IOException,
		...
		int result = source.readData(trackIndex, currentPositionUs,
				formatHolder, sampleHolder, true);
	...
	private boolean feedInputBuffer() throws IOException, ExoPlaybackException {
		...
		try {
			...
			if (sampleEncrypted) {
				MediaCodec.CryptoInfo cryptoInfo = getFrameworkCryptoInfo(
						sampleHolder, adaptiveReconfigurationBytes);
				codec.queueSecureInputBuffer(inputIndex, 0, cryptoInfo,
						presentationTimeUs, 0);
			} else {
				codec.queueInputBuffer(inputIndex, 0, bufferSize,
						presentationTimeUs, 0);
			}
	...
	private static MediaCodec.CryptoInfo getFrameworkCryptoInfo(
		...
		MediaCodec.CryptoInfo cryptoInfo = sampleHolder.cryptoInfo
	...
	private void onInputFormatChanged(FormatHolder formatHolder)
		...
		if (codec != null
				&& canReconfigureCodec(codec, codecIsAdaptive, oldFormat,
						format)) {
	 ...
	 *         otherwise.
	 */
	protected boolean canReconfigureCodec(MediaCodec codec,
			boolean codecIsAdaptive, MediaFormat oldFormat,
	...
	protected boolean isReady() {
		return format != null && !waitingForKeys
				&& ((codec == null && !shouldInitCodec()) // We don't want the
															// codec
						|| outputIndex >= 0 // Or we have an output buffer ready
											// to release
						|| inputIndex < 0 // Or we don't have any input buffers
											// to write to
				|| isWithinHotswapPeriod()); // Or the codec is being hotswapped
	}

	private boolean isWithinHotswapPeriod() {
		return SystemClock.elapsedRealtime() < codecHotswapTimeMs
	...
	private boolean drainOutputBuffer(long timeUs) throws ExoPlaybackException {
		...
		boolean decodeOnly = decodeOnlyPresentationTimestamps
				.contains(outputBufferInfo.presentationTimeUs);
		if (processOutputBuffer(timeUs, codec, outputBuffers[outputIndex],
				outputBufferInfo, outputIndex, decodeOnly)) {
			if (decodeOnly) {
				decodeOnlyPresentationTimestamps
						.remove(outputBufferInfo.presentationTimeUs);
			} else {
	 ...
	 */
	protected abstract boolean processOutputBuffer(long timeUs,
			MediaCodec codec, ByteBuffer buffer,
	...
	private void notifyDecoderInitializationError(
			final DecoderInitializationException e) {
