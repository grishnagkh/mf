 ...
 */
package mf.com.google.android.exoplayer.smoothstreaming;

import android.util.Base64;
import android.util.Pair;
import mf.com.google.android.exoplayer.ParserException;
import mf.com.google.android.exoplayer.parser.mp4.CodecSpecificDataUtil;
import mf.com.google.android.exoplayer.smoothstreaming.SmoothStreamingManifest.ProtectionElement;
...
public class SmoothStreamingManifestParser {
	 ...
	 *             manifest.
	 */
	public SmoothStreamingManifest parse(InputStream inputStream,
			String inputEncoding) throws XmlPullParserException, IOException,
			ParserException {
		XmlPullParser xmlParser = xmlParserFactory.newPullParser();
		xmlParser.setInput(inputStream, inputEncoding);
		SmoothStreamMediaParser smoothStreamMediaParser = new SmoothStreamMediaParser(
				null);
		return (SmoothStreamingManifest) smoothStreamMediaParser
				.parse(xmlParser);
	}
	...
	private static abstract class ElementParser {
		...
		public final Object parse(XmlPullParser xmlParser)
				...
				case XmlPullParser.START_TAG:
					...
					if (tag.equals(tagName)) {
						...
						if (handleChildInline(tagName)) {
							...
							addChild(newChildParser(this, tagName).parse(
									xmlParser));
						}
					}
					break;
		 ...
		 * @throws ParserException
		 */
		protected void parseStartTag(XmlPullParser xmlParser)
				throws ParserException {
			// Do nothing.
		}
		...
		protected final String parseRequiredString(XmlPullParser parser,
				String key) throws MissingFieldException {
		...
		protected final int parseInt(XmlPullParser parser, String key,
				int defaultValue) throws ParserException {
			String value = parser.getAttributeValue(null, key);
			if (value != null) {
				try {
		...
		protected final long parseLong(XmlPullParser parser, String key,
				long defaultValue) throws ParserException {
			String value = parser.getAttributeValue(null, key);
			if (value != null) {
				try {
	...
	private static class SmoothStreamMediaParser extends ElementParser {
		...
		@Override
		public Object build() {
			StreamElement[] streamElementArray = new StreamElement[streamElements
					.size()];
			streamElements.toArray(streamElementArray);
			return new SmoothStreamingManifest(majorVersion, minorVersion,
					timeScale, duration, lookAheadCount, protectionElement,
					streamElementArray);
		}
	...
	private static class StreamElementParser extends ElementParser {
		...
		private void parseStreamFragmentStartTag(XmlPullParser parser)
				throws ParserException {
			startTimes[chunkIndex] = parseLong(parser, KEY_FRAGMENT_START_TIME,
					-1L);
			if (startTimes[chunkIndex] == -1L) {
				if (chunkIndex == 0) {
					...
					// Infer the start time from the previous chunk's start time
					// and duration.
					startTimes[chunkIndex] = startTimes[chunkIndex - 1]
							+ previousChunkDuration;
				} else {
					// We don't have the start time, and we're unable to infer
					// it.
					throw new ParserException("Unable to infer start time");
				}
			}
			previousChunkDuration = parseLong(parser, KEY_FRAGMENT_DURATION,
					-1L);
			chunkIndex++;
		}

		private void parseStreamElementStartTag(XmlPullParser parser)
				throws ParserException {
		...
		public Object build() {
			...
			return new StreamElement(type, subType, timeScale, name,
					qualityLevels, url, maxWidth, maxHeight, displayWidth,
					displayHeight, language, trackElements, startTimes);
		}
	...
	private static class TrackElementParser extends ElementParser {
		...
		public void parseStartTag(XmlPullParser parser) throws ParserException {
			...
			if (value != null && value.length() > 0) {
				...
				byte[][] split = CodecSpecificDataUtil
						.splitNalUnits(codecPrivateData);
				if (split == null) {
					csd.add(codecPrivateData);
				} else {
					for (int i = 0; i < split.length; i++) {
						Pair<Integer, Integer> spsParameters = CodecSpecificDataUtil
								.parseSpsNalUnit(split[i]);
		...
		private byte[] hexStringToByteArray(String hexString) {
			...
			for (int i = 0; i < data.length; i++) {
				...
				data[i] = (byte) ((Character.digit(
						hexString.charAt(stringOffset), 16) << 4) + Character
						.digit(hexString.charAt(stringOffset + 1), 16));
			}
			return data;
		}
		...
		public Object build() {
			...
			return new TrackElement(index, bitrate, fourCC, csdArray, profile,
					level, maxWidth, maxHeight, samplingRate, channels,
					packetSize, audioTag, bitPerSample, nalUnitLengthField,
					content);
		}
