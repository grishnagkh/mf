...
public class MediaCodecVideoTrackRenderer extends MediaCodecTrackRenderer {
	 ...
	 */
	public MediaCodecVideoTrackRenderer(SampleSource source,
	...
	public MediaCodecVideoTrackRenderer(SampleSource source,
		...
		this(source, drmSessionManager, playClearSamplesWithoutKeys,
				videoScalingMode, allowedJoiningTimeMs, null, null, -1);
	}
	...
	protected void onEnabled(long startTimeUs, boolean joining) {
		...
		if (joining && allowedJoiningTimeUs > 0) {
			joiningDeadlineUs = SystemClock.elapsedRealtime() * 1000L
	...
	protected boolean isReady() {
		if (super.isReady() && (renderedFirstFrame || !codecInitialized())) {
			...
			// The joining deadline has been exceeded. Give up and clear the
			// deadline.
			joiningDeadlineUs = -1;
			return false;
		}
	...
	private void setSurface(Surface surface) throws ExoPlaybackException {
		...
		int state = getState();
		if (state == TrackRenderer.STATE_ENABLED
	...
	protected void onOutputFormatChanged(android.media.MediaFormat format) {
		boolean hasCrop = format.containsKey(KEY_CROP_RIGHT)
				&& format.containsKey(KEY_CROP_LEFT)
				&& format.containsKey(KEY_CROP_BOTTOM)
				&& format.containsKey(KEY_CROP_TOP);
		currentWidth = hasCrop ? format.getInteger(KEY_CROP_RIGHT)
				- format.getInteger(KEY_CROP_LEFT) + 1 : format
				.getInteger(android.media.MediaFormat.KEY_WIDTH);
		currentHeight = hasCrop ? format.getInteger(KEY_CROP_BOTTOM)
				- format.getInteger(KEY_CROP_TOP) + 1 : format
				.getInteger(android.media.MediaFormat.KEY_HEIGHT);
	}
	...
	private void renderOutputBuffer(MediaCodec codec, int bufferIndex) {
		if (lastReportedWidth != currentWidth
	...
	private void notifyAndResetDroppedFrameCount() {
		if (eventHandler != null && eventListener != null
			...
			final long elapsedToNotify = now
			...
			eventHandler.post(new Runnable() {
				@Override
				public void run() {
					eventListener.onDroppedFrames(countToNotify,
							elapsedToNotify);
				}
			});
		}
