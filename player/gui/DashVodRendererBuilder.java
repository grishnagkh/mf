...
import mf.com.google.android.exoplayer.dash.DashMp4ChunkSource;
import mf.com.google.android.exoplayer.dash.mpd.AdaptationSet;
import mf.com.google.android.exoplayer.dash.mpd.MediaPresentationDescription;
import mf.com.google.android.exoplayer.dash.mpd.MediaPresentationDescriptionFetcher;
import mf.com.google.android.exoplayer.dash.mpd.Period;
...
/* package */class DashVodRendererBuilder implements RendererBuilder,
	...
	public DashVodRendererBuilder(PlayerActivity playerActivity,
	...
	public void buildRenderers(RendererBuilderCallback callback) {
		...
		MediaPresentationDescriptionFetcher mpdFetcher = new MediaPresentationDescriptionFetcher(
				this);
		mpdFetcher.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, url,
				contentId);
	}
	...
	@Override
	public void onManifest(String contentId,
		...
		for (int i = 0; i < period.adaptationSets.size(); i++) {
			...
			for (int j = 0; j < adaptationSet.representations.size(); j++) {
				Representation representation = adaptationSet.representations
						.get(j);
				if (audioRepresentation == null
						&& adaptationSetType == AdaptationSet.TYPE_AUDIO) {
		...
		Representation[] videoRepresentations = new Representation[videoRepresentationsList
				.size()];
